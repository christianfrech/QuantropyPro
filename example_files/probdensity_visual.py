...
Description: Contains methods to generate adjacency matrices for peptide-water networks 
and probability density visuzlizations 
"""
...
class Adj_Mats(object):
    ...
    Methods:
        ...
        get_elec_adj: Used to calculate spacings between eigenvalues of electron adjacency
    """
    ...
    def get_spacings(self,types):
        ...
        allframes_spacing=np.zeros((len(eigenvalues), len(eigenvalues[0])))
        ...
        if types=='all':
            ...
            for frame in range(len(self.elec_adjacency_graphs)):
                ...
                for i in range(len(eigenvalues[frame])):
                    for j in range(len(eigenvalues[frame][i])):
...
#Create list of interactiono distance cutoffs
...
"""
Set the PDB files used, the number of bins for probability density 
plots type of calcultion 
Types:
'gif': generates a bivariate probability density plot gif (each frame of gif is a different interaction 
distance cutoff, each gif frame varies with both PDB frame and spacing value), must match dimension of
frames and spacing for well-formatted heat map
'median': generates a 2D probabillity density plot with overlaid probability distributions corresponding
to different frames/interaction distances 
"""
...
spacing_list = []
lowerlimit=1
#upperlimit_list = np.arange(lowerlimit,10,4)
upperlimit_list = [5]
frame_period = 10000
...
append the spacings between eigenvalues full list (also create histogram for gif)
"""
...
for upperlimit in upperlimit_list:
    if __name__ == "__main__":
        ...
        batch_spacings=batch.get_spacings(types)
        spacinginfo.extend(batch_spacings)
        file = open(filename2)
        batch2 = Adj_Mats(filename2, batchframesextend)
        batch2_spacings=batch2.get_spacings(types)
        spacinginfo.extend(batch2_spacings)


        if (types=='median'):
            spacing_list.append(spacinginfo)
            
        elif (types == 'gif'):
            x = framesindices 
            x_coord = np.repeat(x, len(spacinginfo[0]))
            spacing_array = (spacinginfo)
            allhists = np.ravel(spacing_array)

            # Create a figure for plotting the data as a bivariate heat map.
            fig, ax = plt.subplots(1,1)
            weighting = np.ones_like(allhists) / len(cp.asnumpy(spacinginfo[0]))
            ax.hist2d(allhists, x_coord, bins=numberofbins, weights=weighting)
            ax.set_xlabel('Spacing')
            ax.set_ylabel('Frames Elapsed (10ps)')
            filename='3Dpep_probdensity3dplot_nores_'+str(upperlimit)+'.png'
...
if (types == 'median'):
    """
    Creating 2D probability density plots of spacings for various frames 
    """
    ...
    for dist in range(len(Z)):
        for frame in range(len(Z[dist])):
            spacingbinvalues = binSizes(Z[dist][frame], numberofbins)
            weighting = np.ones_like(Z[dist][frame]) / len(Z[dist][frame])
            
            #ENTER WEIGHTINGS BACK IN FOR PROBABILITY
            ax.hist(Z[dist][frame], bins = spacingbinvalues, alpha=0.5, weights=weighting)
    
    ax.legend(framesindices, loc=2)
    plt.savefig('probdensity_median_test.png', dpi=95)
    plt.show()

elif (types == 'gif'):
    """
    Creating 3D gif of probability distribution of spacings heatmaps (each gif frame a different interaction distance cutoff)
    """ 
    images=[]
    for name in filenames:
        images.append(imageio.imread(name))
    imageio.mimsave('probdensity_test.gif', images, duration=1)
